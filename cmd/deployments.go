...
func getDeployments(currentContextPath string, defaultConfigNamespace string, resourceName string, allNamespacesFlag bool, outputFlag string, showLabels bool, jsonPathTemplate string, allResources bool) bool {
	...
	for _, _namespace := range namespaces {
		...
		for _, Deployment := range _Items.Items {
			...
			//age
			ResourceFile, _ := os.Stat(CurrentNamespacePath + "/apps/deployments.yaml")
			t2 := ResourceFile.ModTime()
			t1 := Deployment.GetCreationTimestamp()
			...
			_list := []string{Deployment.Namespace, DeploymentName, ready, upToDateReplicas, availableReplicas, diffTimeString, containers, images}
